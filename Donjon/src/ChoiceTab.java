...
public class ChoiceTab {
	...
	public boolean makeChoice() throws IOException { // main function. Used to show the choices and select one. Boolean
		...
		case "takeH":// take it from "from"
			...
			System.out.println();
			choice_selected.from.loseHp(choice_selected.target.getUsingWeapon().pa); // we take the damages of the main
																						// weapon
			return true;
		...
		case "equipC": // equip with the chest content
			choice_selected.from.equip(choice_selected.chest.getContent());
			return true;
